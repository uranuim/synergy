...
import time
import cv2
from test import HandGestureRecognizer  # Import the HandGestureRecognizer class
...
# Connect the client sockets
server_ip = '192.168.223.156'
video_client_socket.connect((server_ip, 9999))
audio_client_socket.connect((server_ip, 10000))
chat_client_socket.connect((server_ip, 10001))
...
def start_chat_window():
    ...
    chat_input.bind("<Return>", lambda event: send_chat(chat_input.get()))

    predict_button = tk.Button(root, text="Start Predicting", command=start_prediction)
    predict_button.pack(padx=10, pady=10)

    root.mainloop()

# Function to start hand gesture prediction
def start_prediction():
    def predict_and_type():
        recognizer = HandGestureRecognizer()
        try:
            while True:
                prediction = recognizer.get_prediction()
                if prediction:
                    chat_input.insert(tk.END, prediction)
                time.sleep(0.5)
        finally:
            recognizer.release()

    prediction_thread = threading.Thread(target=predict_and_type)
