...
def start_chat_window():
    ...
    # Button to start predicting and sending alphabets to chat
...
def start_prediction():
    def predict_and_send():
        ...
        try:
            while True:
                # Get prediction from test.py
                prediction = recognizer.get_prediction()
                if prediction:
                    send_chat(prediction)
                time.sleep(2)  # Wait for 2 seconds before next prediction
        finally:
            recognizer.release()

    prediction_thread = threading.Thread(target=predict_and_send)
